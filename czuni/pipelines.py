...
class CzuniPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('czuni.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `czuni`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM czuni WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO czuni (date, title, link, content)"
